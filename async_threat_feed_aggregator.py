...
# Expanded list of reliable threat intel RSS feeds
FEEDS: Dict[str, str] = {
...
async def gather_entries() -> List[Dict]:
    entries: List[Dict] = []
...
def display_entries(entries: List[Dict], seen_links: set):
    table = Table(
        title=f"Threat Intelligence Feeds ({datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')})"
    )
